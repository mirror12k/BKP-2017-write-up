<?PHP
// to bypass this, we need the server to load a SCRIPT_FILENAME that is not acu_phpaspect.php
// the server is configured to autoload this page on all files, so we just need to access index.php with the proper headers
if(strtolower(basename($_SERVER["SCRIPT_FILENAME"])) === strtolower(basename(__FILE__))) {
	header("HTTP/1.0 404 Not found") ;
	die() ;
}
// need to set a ACUNETIX_ASPECT header to 'enabled'
$_ENV['_AAS0'] =  (isset($_SERVER["HTTP_ACUNETIX_ASPECT"]) && $_SERVER["HTTP_ACUNETIX_ASPECT"] === "enabled") ;
if($_ENV['_AAS0']) {
	$_ENV['_AAS0'] = false ;
	// need to set a ACUNETIX_ASPECT_PASSWORD header to '4faa9d4408780ae071ca2708e3f09449'
...
if($_ENV['_AAS0']) {
	...
	$_AAS12 = 0 ;
	if($_ENV["_AAS9"] !== false) {
		while(file_exists(sprintf("%s_%.3n%s", $_ENV["_AAS9"], $_AAS12, ".txt")))$_AAS12++ ;
	...
	if($_ENV["_AAS31"]) {
		...
		if(substr($_ENV["_AAS31"], -1) !== "/")$_ENV["_AAS31"] .= "/" ;
	}
	$_ENV["_AAS34"] = (bool)$_ENV["_AAS31"] ;
	$_ENV["_AAS35"] = ini_get("error_reporting") ;
	$_ENV["_AAS36"] = array ( E_ERROR => "Error", E_WARNING => "Warning", E_PARSE => "Parsing Error", E_NOTICE => "Notice", E_CORE_ERROR => "Core Error", E_CORE_WARNING => "Core Warning", E_COMPILE_ERROR => "Compile Error", E_COMPILE_WARNING => "Compile Warning", E_USER_ERROR => "User Error", E_USER_WARNING => "User Warning", E_USER_NOTICE => "User Notice", E_STRICT => "Runtime Notice", E_RECOVERABLE_ERROR => "Catchable Fatal Error" ) ;
}
function _AAS37($_AAS38, $_AAS39, $_AAS40, $_AAS41)  {
	...
	if ($_AAS38 == E_USER_ERROR || $_AAS38 == E_ERROR) _AAS43() ;
	if (is_array($_ENV['_AAS11']) && count($_ENV['_AAS11']) > 0)  {
		...
		elseif (is_array($_ENV['_AAS11'][0]) && is_object($_ENV['_AAS11'][0][0]))  {
			if (!isset($_ENV['_AAS11'][1]) || ($_ENV['_AAS11'][1] & $_AAS38 != 0))  {
				$_AAS44 = func_get_args() ;
				call_user_func_array(array($_ENV['_AAS11'][0][0], $_ENV['_AAS11'][0][1]), $_AAS44) ;
			}
		}
	}
	else if (($_AAS38 & $_ENV["_AAS35"]) != 0)  {
		switch ($_AAS38)  {
			case E_USER_ERROR: case E_RECOVERABLE_ERROR: case E_ERROR: echo "<b>Fatal error</b>: $_AAS39 in <b>" . $_ENV['_AAS29'] . "</b> on line <b>$_AAS41</b><br />\n" ;
			break ;
			case E_USER_WARNING: case E_WARNING: echo "<b>Warning</b>: $_AAS39 in <b>" . $_ENV['_AAS29'] . "</b> on line <b>$_AAS41</b><br />\n" ;
			break ;
			case E_USER_NOTICE: case E_NOTICE: case E_STRICT: break ;
			default: echo "<b>Unknown error type $_AAS38</b>: $_AAS39 in <b>" . $_ENV['_AAS29'] . "</b> on line <b>$_AAS41<b><br />\n" ;
			break ;
		}
	}
	return true ;
...
function _AAS45($_AAS46, $_AAS47)  {
	switch (count($_AAS47))  {
		case 0: return $_AAS46() ;
		break ;
		case 1: return $_AAS46($_AAS47[0]) ;
		break ;
		case 2: return $_AAS46($_AAS47[0], $_AAS47[1]) ;
		break ;
		case 3: return $_AAS46($_AAS47[0], $_AAS47[1], $_AAS47[2]) ;
		break ;
		case 4: return $_AAS46($_AAS47[0], $_AAS47[1], $_AAS47[2], $_AAS47[3]) ;
		break ;
		case 5: return $_AAS46($_AAS47[0], $_AAS47[1], $_AAS47[2], $_AAS47[3], $_AAS47[4]) ;
		break ;
		default: return call_user_func_array($_AAS46, $_AAS47) ;
		break ;
	}
}
...
function _AAS55($_AAS56, $_AAS57, $_AAS58, $_AAS59, $_AAS60, $_AAS61)  {
	...
	if (strpos($_AAS61, "acu_phpaspect.php") !== false || strpos($_AAS61, _AAS7) !== false || strpos($_AAS61, _AAS8) !== false ) return "" ;
	$_AAS51 = false ;
	if (($_AAS62 = realpath($_AAS61)) === false || !file_exists($_AAS62))  {
		...
		if ($_AAS51 !== false)  {
			...
			if ($_AAS61[0] !== "." && $_AAS61[0] !== "/" && $_AAS61[0] !== "\\")  {
				...
				if ($_AAS62 !== false && file_exists($_AAS62)) $_AAS66 = false ;
			}
			if ($_AAS66)  {
	...
	if ($_AAS51 === false) $_AAS51 = @file_get_contents($_AAS62, true) ;
...
function _AAS18($_AAS61, $_AAS75, $_AAS60, $_AAS77)  {
	if (is_array($_AAS77) && count($_AAS77) == 1 && (strpos($_AAS77[0], _AAS7 . 'FILE') !== false || strpos($_AAS77[0], _AAS8 . 'FILE') !== false))  {
		if (strpos($_AAS77[0], _AAS7 . 'FILECREATE') !== false || strpos($_AAS77[0], _AAS8 . 'FILECREATE') !== false) return false ;
		else return true ;
	}
	else  {
...
function _AAS24($_AAS61, $_AAS75, $_AAS60, $_AAS77)  {
	if (is_array($_AAS77) && count($_AAS77) >= 3)  {
		$_AAS78 = _AAS79() ;
		if ($_AAS78 != "") $_AAS80 = Array("\"$_AAS60\" was called.\r\n$_AAS78") ;
		else $_AAS80 = Array("\"$_AAS60\" was called.") ;
		if ($_AAS77[0] == _AAS7)  {
			array_push($_AAS80, "Regex pattern is controllable") ;
			_AAS50("Preg_Replace_Warning", Array($_AAS77[0], $_AAS77[1], substr($_AAS77[2], 0, 128)), $_AAS61, $_AAS75, $_AAS80) ;
		}
...
function _AAS22($_AAS61, $_AAS75, $_AAS60, $_AAS77)  {
	if (count($_AAS77) > 0 && is_resource($_AAS77[0]) && defined("CURLINFO_EFFECTIVE_URL"))  {
		...
		_AAS50("CURL_Exec", Array($_AAS84), $_AAS61, $_AAS75, $_AAS80) ;
	}
	return _AAS45($_AAS60, $_AAS77) ;
}
	...
	if ($_AAS85 !== false)  {
		if ($_AAS85[2] !== false && function_exists($_AAS85[2]))  {
			...
			for ($_AAS12 = 0 ;
				$_AAS12 < count($_AAS77) ;
				$_AAS12++)  {
				if ($_AAS88 && ((is_string($_AAS77[$_AAS12]) && strpos($_AAS77[$_AAS12], _AAS7) !== false) || (is_string($_AAS77[$_AAS12]) && strpos($_AAS77[$_AAS12], _AAS8) !== false)) ) $_AAS88 = false ;
			if (in_array($_AAS12 + 1, $_AAS85[1])) array_push($_AAS89, substr($_AAS77[$_AAS12], 0, 1024 * 1024)) ;
		}
		$_AAS78 = _AAS79() ;
		if ($_AAS78 != "") $_AAS80 = Array("\"$_AAS87\" was called.\r\n$_AAS78") ;
		else $_AAS80 = Array("\"$_AAS87\" was called.") ;
		for ($_AAS12 = 3 ;
			$_AAS12 < count($_AAS85) ;
			$_AAS12++)  {
			if (isset($_AAS85[$_AAS12]))  {
		...
		if ($_AAS88) return _AAS45($_AAS87, $_AAS77) ;
		else return false ;
	}
}
else  {
...
function _AAS26($_AAS61, $_AAS75, $_AAS90, $_AAS60, $_AAS77)  {
	...
	if ($_AAS85 !== false && isset($_AAS85[3]) && is_object($_AAS90) && ($_AAS90 instanceof $_AAS85[3]))  {
		...
		for ($_AAS12 = 0 ;
			$_AAS12 < count($_AAS77) ;
			$_AAS12++)  {
			if (in_array($_AAS12 + 1, $_AAS85[1])) array_push($_AAS89, $_AAS77[$_AAS12]) ;
	}
	$_AAS80 = Array("\"$_AAS60\" member function was called.") ;
	for ($_AAS12 = 4 ;
		$_AAS12 < count($_AAS85) ;
		$_AAS12++)  {
		if (isset($_AAS85[$_AAS12]))  {
...
if ($_AAS88)  {
	return call_user_func_array(array($_AAS90, $_AAS60), $_AAS77) ;
}
else  {
	return false ;
}
...
function _AAS91($_AAS61, $_AAS75, $_AAS92, $_AAS93)  {
	...
	if (isset($_AAS85))  {
		_AAS50("Var_Access", Array($_AAS85[0], $_AAS93), $_AAS61, $_AAS75) ;
	}
	return $_AAS93 ;
}
function _AAS17($_AAS61, $_AAS75, $_AAS60, $_AAS77)  {
	...
	if (strpos($_AAS95, 'w') !== false || strpos($_AAS95, 'a') !== false || strpos($_AAS95, 'x') !== false)  {
		_AAS50('Create_File', Array($_AAS94), $_AAS61, $_AAS75, $_AAS80) ;
	}
	else  {
		_AAS50('File_Open', Array($_AAS94), $_AAS61, $_AAS75, $_AAS80) ;
	}
...
function _AAS25($_AAS61, $_AAS75, $_AAS60, $_AAS77)  {
	...
	if (isset($_AAS97))  {
		...
		if (($_AAS100 = strpos($_AAS99, _AAS7)) !== false)  {
			...
			_AAS50('File_Write', Array($_AAS97, $_AAS99), $_AAS61, $_AAS75, $_AAS80) ;
		}
...
function _AAS23($_AAS61, $_AAS75, $_AAS60, $_AAS77)  {
	...
	if ($_AAS96 && ($_AAS62 = realpath($_AAS77[1])) !== false)  {
		...
		if (substr($_AAS101, -1, 1) !== "/") $_AAS101 .= "/" ;
		$_AAS102 = false ;
		if (strpos($_AAS62, $_AAS101) === 0)  {
			$_AAS102 = substr($_AAS62, _AAS53($_AAS101) - 1) ;
		}
		if ($_AAS102)  {
			_AAS50('File_Upload', Array($_AAS102), $_AAS61, $_AAS75, Array("\"$_AAS62\" was uploaded (platform PHP).\r\n$_AAS78")) ;
		}
		else  {
			_AAS50('Create_File', $_AAS77, $_AAS61, $_AAS75, Array("\"$_AAS60\" was called.\r\n$_AAS78")) ;
		}
	}
	return $_AAS96 ;
}
	...
	if (is_dir($_AAS114) && $_AAS116 = @opendir($_AAS114))  {
		while (($_AAS107 = readdir($_AAS116)) !== false)  {
			if (is_dir($_AAS114 . $_AAS107) && $_AAS107 != "." && $_AAS107 != "..")  {
				...
				if ($_AAS115) $_AAS96 = array_merge($_AAS96, _AAS113($_AAS114 . $_AAS107, $_AAS115)) ;
			}
...
function _AAS117()  {
	$_AAS118 = Array() ;
	if (($_AAS119 = ini_get('display_errors')) != 0) array_push($_AAS118, 'display_errors=' . $_AAS119) ;
	if (strtolower(ini_get('register_globals')) == "on" || ini_get('register_globals') == 1) array_push($_AAS118, 'register_globals_on=on') ;
	if (strtolower(ini_get('magic_quotes_gpc')) == "off" || ini_get('register_globals') == 0) array_push($_AAS118, 'magic_gpc_off=off') ;
	if ((bool)ini_get('allow_url_fopen')) array_push($_AAS118, 'allow_url_fopen_on=On') ;
	if ((bool)ini_get('allow_url_include')) array_push($_AAS118, 'allow_url_include_on=On') ;
	if ((bool)ini_get('session.use_trans_sid')) array_push($_AAS118, 'session.use_trans_sid_on=On') ;
	if (ini_get('open_basedir') == '') array_push($_AAS118, 'open_basedir_not_set=') ;
	if ((bool)ini_get('enable_dl') && (bool)ini_get('safe_mode')) array_push($_AAS118, 'enable_dl_safe_mode_on=') ;
	array_push($_AAS118, "php_version=" . phpversion()) ;
	if (count($_AAS118) > 0) _AAS50("Aspect_Alerts", $_AAS118, $_SERVER["SCRIPT_FILENAME"], 0) ;
}
function _AAS79($_AAS120 = true)  {
	...
	if ($_AAS120)  {
		while ($_AAS122 >= 0 && $_AAS121[$_AAS122]["function"] === "eval") $_AAS122-- ;
		if ($_AAS122 <= 0) return "" ;
	}
	$_AAS96 = "" ;
	$_AAS93 = 1 ;
	for ($_AAS12 = 0 ;
		$_AAS12 <= $_AAS122 ;
		$_AAS12++)  {
		if ($_AAS120 && (strpos($_AAS121[$_AAS12]["function"], "_AAS") !== false || strpos($_AAS121[$_AAS12]["function"], "call_user_func_array") !== false) || strpos($_AAS121[$_AAS12]["function"], "call_user_method_array") !== false ) continue ;
	...
	if (isset($_AAS121[$_AAS12]["args"]))  {
		...
		$_AAS87 .= "[$_AAS125] " ;
		switch ($_AAS125)  {
			case "array": $_AAS87 .= "count=" . count($_AAS121[$_AAS12]["args"][$_AAS124]) ;
			break ;
			case "object": $_AAS87 .= "class=" . get_class($_AAS121[$_AAS12]["args"][$_AAS124]) ;
			break ;
			case "string": $_AAS87 .= "\"" . str_replace(Array("\r\n", "\n", "\r"), Array("\\n", "\\n", "\\n"), $_AAS121[$_AAS12]["args"][$_AAS124]) . "\"" ;
			break ;
			case "boolean": $_AAS87 .= $_AAS121[$_AAS12]["args"][$_AAS124] ? "true" : "false" ;
			break ;
			default: $_AAS87 .= $_AAS121[$_AAS12]["args"][$_AAS124] ;
		}
		if ($_AAS124 < count($_AAS121[$_AAS12]["args"]) - 1) $_AAS87 .= ", " ;
	}
	$_AAS87 .= ")" ;
}
else  {
...
function _AAS126()  {
	...
	if ($_AAS98 != "") $_AAS98 = substr($_AAS98, 0, -1) ;
	return $_AAS98 ;
}
class _AAS70  {
	private $_AAS128 ;
	...
	private function _AAS138($_AAS139) {
		if(is_string($_AAS139)) {
			...
			switch($_AAS139[0]) {
				case T_FILE: return '"'.$this->_AAS128.'"' ;
				break ;
				case T_DIR: return '"'.$this->_AAS137.'"' ;
				break ;
				default: return $_AAS139[1] ;
			}
		}
	}
	...
	private function _AAS142(&$_AAS141) {
		...
		switch($_AAS139[0]) {
			case T_INCLUDE: $_AAS60 = "include" ;
			$_AAS58 = "false" ;
			$_AAS59 = "false" ;
			break ;
			case T_INCLUDE_ONCE: $_AAS60 = "include_once" ;
			$_AAS58 = "false" ;
			$_AAS59 = "true" ;
			break ;
			case T_REQUIRE: $_AAS60 = "require" ;
			$_AAS58 = "true" ;
			$_AAS59 = "false" ;
			break ;
			case T_REQUIRE_ONCE: $_AAS60 = "require_once" ;
			$_AAS58 = "true" ;
			$_AAS59 = "true" ;
			break ;
		}
		...
		while($_AAS141<$this->_AAS130) {
			...
			if(is_string($_AAS139)) {
				if($_AAS139 === "("){$_AAS47++; }
				elseif($_AAS139 === ")"){$_AAS47--; }
				elseif($_AAS139 === "?"){$_AAS144 = true; }
				elseif($_AAS139 === ":") {
					if($_AAS144 === false)  {
						$_AAS141-- ;
						break ;
	...
	private function _AAS146(&$_AAS141) {
		while($_AAS141<$this->_AAS130) {
			$_AAS139 = $this->_AAS129[$_AAS141] ;
			if(is_array($_AAS139) && ( $_AAS139[0] === T_COMMENT || $_AAS139[0] === T_ML_COMMENT || $_AAS139[0] === T_DOC_COMMENT || $_AAS139[0] === T_WHITESPACE )) $_AAS141++ ;
			else break ;
		}
	}
	private function _AAS147(&$_AAS141, $_AAS148, $_AAS149, &$_AAS150, &$_AAS93) {
		...
		while($_AAS151<$this->_AAS130)  {
			...
			if($_AAS139 === $_AAS148)$_AAS47++ ;
			if($_AAS139 === $_AAS149)$_AAS47-- ;
			if($_AAS47<=0)break ;
			if($_AAS139 === ";") {
				return false ;
			}
			else  {
				if(is_array($_AAS139) && $_AAS139[0] !== T_COMMENT && $_AAS139[0] !== T_ML_COMMENT && $_AAS139[0] !== T_DOC_COMMENT && $_AAS139[0] !== T_WHITESPACE && $_AAS139[0] !== T_CONSTANT_ENCAPSED_STRING ) $_AAS150 = false ;
	...
	private function _AAS156(&$_AAS141)  {
		...
		while ($_AAS141 < $this->_AAS130)  {
			...
			if ($_AAS139 === '(')  {
				$_AAS47++ ;
				if ($_AAS47 == 0) $_AAS47 = 1 ;
			}
			elseif ($_AAS139 == ')' && --$_AAS47 == 0)  {
				return $_AAS96 . ')' ;
			}
			if (is_array($_AAS139)) $_AAS96 .= $_AAS139[1] ;
			else $_AAS96 .= $_AAS139 ;
			$_AAS141++ ;
		}
	}
		...
		while($_AAS151<$this->_AAS130) {
			...
			if($_AAS139 === "{" || $_AAS139 === "[") {
				...
				if($_AAS162 && isset($_AAS85) && $_AAS85 !== false) {
					if($_AAS150)_AAS50("Var_Reference", Array($_AAS85[0], trim($_AAS93, "\"'")), $this->_AAS128, $_AAS75) ;
	...
	private function _AAS145(&$_AAS141) {
		...
		if(is_string($_AAS139)) {
			switch($_AAS139) {
				case "\"": $_AAS96 = $this->_AAS140($_AAS141) ;
				break ;
				default: $_AAS96 .= $this->_AAS138($_AAS139) ;
			}
		}
		else  {
			switch($_AAS139[0]) {
				case T_INCLUDE: case T_INCLUDE_ONCE: case T_REQUIRE: case T_REQUIRE_ONCE: $_AAS96 .= $this->_AAS142($_AAS141) ;
				break ;
				case T_VARIABLE: $_AAS96 .= $this->_AAS161($_AAS141) ;
				break ;
				case T_FUNCTION: $_AAS96 .= $this->_AAS156($_AAS141) ;
				break ;
				case T_NEW: $_AAS96 .= $this->_AAS157($_AAS141, "new") ;
				break ;
				case T_CLASS: $_AAS96 .= $this->_AAS157($_AAS141, "class") ;
				break ;
				case T_DOUBLE_COLON: $_AAS96 .= $this->_AAS157($_AAS141, "::") ;
				break ;
				case T_OBJECT_OPERATOR: $_AAS96 .= $this->_AAS157($_AAS141, "->") ;
				break ;
				case T_EXTENDS: $_AAS96 .= $this->_AAS157($_AAS141, "extends") ;
				break ;
				case T_STRING: $_AAS96 .= $this->_AAS154($_AAS141) ;
				break ;
				case T_WHITESPACE: $_AAS96 .= " " ;
				break ;
				case T_EVAL: $_AAS96 .= $this->_AAS164($_AAS141) ;
				break ;
				case T_EXIT: $_AAS96 .= $this->_AAS165($_AAS141) ;
				break ;
				case T_HALT_COMPILER: $_AAS141 = $this->_AAS130 ;
				break ;
				case T_START_HEREDOC: case T_END_HEREDOC: $_AAS96 .= $_AAS139[1]."\n" ;
				break ;
				default: if($_AAS139[0] === T_OPEN_TAG) {
					$this->_AAS132 = true ;
				}
	...
	public function _AAS71($_AAS166) {
		...
		if($this->_AAS133) {
			...
			if($_AAS1 = @fopen($this->_AAS134, "rb")) {
				...
				if($_AAS168>0)$this->_AAS73 = @fread($_AAS1, $_AAS168) ;
				else $this->_AAS73 = "" ;
			}
			else  {
		...
		if($this->_AAS131) {
			if($this->_AAS132)$this->_AAS73 .= "return true;" ;
			else $this->_AAS73 .= "<?PHP return true;?>" ;
		}
	}
}
if($_ENV["_AAS0"]) {
	...
	if(isset($_SERVER["HTTP_ACUNETIX_ASPECT_QUERIES"])) {
		...
		for($_AAS12=0;$_AAS12<count($_AAS169);$_AAS12++) {
			switch(strtolower($_AAS169[$_AAS12])) {
				case "filelist": if(strcasecmp(basename($_ENV['_AAS29']), basename($_SERVER["PHP_SELF"]))==0) {
					_AAS50("File_List", _AAS113($_ENV['_AAS28'], true), $_ENV['_AAS29'], 0) ;
				}
				break ;
				case "aspectalerts": _AAS117() ;
				break ;
			}
		}
	}
